...
{
    /// <summary>
    /// The switch is <c>/select,</c> — a comma, no space, and the path in the same argument.
    /// <c>/select &lt;path&gt;</c> opens Documents instead, silently, which on a machine none
    /// of us runs is a bug nobody would find until somebody reported that the button went to
    /// the wrong place.
    /// </summary>
    [Fact]
    public void Windows_asks_explorer_to_select_the_file()
    {
        var command = Files.RevealCommand(@"C:\Users\dave\.cairn\packs\anego\data\ModConfig\bedspawn.json",
                                          HostOs.Windows);

        Assert.NotNull(command);
        Assert.Equal("explorer.exe", command!.Value.Exe);
        Assert.Equal([@"/select,C:\Users\dave\.cairn\packs\anego\data\ModConfig\bedspawn.json"],
                     command.Value.Args);
    }
    ...
    {
        var command = Files.RevealCommand("/Users/dave/.cairn/packs/anego/data/ModConfig/bedspawn.json",
                                          HostOs.MacOs);

        Assert.NotNull(command);
        Assert.Equal("open", command!.Value.Exe);
        Assert.Equal(["-R", "/Users/dave/.cairn/packs/anego/data/ModConfig/bedspawn.json"],
                     command.Value.Args);
    }
